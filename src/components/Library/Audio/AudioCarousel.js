...
import {
  ...
  CardCategory,
  CardInfo,
} from "../../styles/AudioCarousel.Elements";
        ...
        <ReviewSlider {...sliderSettings} ref={setSliderRef}>
          {dataSource.map((ds, id) => {
            return (
              <CarouselCard key={id}>
                <CardInfo>
                  <CardCategory>{ds.audioType}</CardCategory>
                  <CardHeading>{ds.audioAuthor}</CardHeading>
                  <CardInformation>
                    <CardDatePublished>
                      (Added {ds.datePublished})
                    </CardDatePublished>
                  </CardInformation>
                  <a href={ds.audioURL} target="_blank" rel="noreferrer">
