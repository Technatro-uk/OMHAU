...
import {
  ...
  CardCategory,
  CardInfo,
} from "../../styles/StoriesCarousel.Elements";

const StoriesCarousel = ({ dataSource }) => {
        ...
        <ReviewSlider {...sliderSettings} ref={setSliderRef}>
          {dataSource.map((ds, id) => {
            return (
              <CarouselCard key={id}>
                <CardInfo>
                  <CardCategory>{ds.storyCategory}</CardCategory>
                  <CardHeading>{ds.storyAuthor}</CardHeading>
                  <CardInformation>
                    <CardDatePublished>
                      (Added {ds.datePublished})
                    </CardDatePublished>
                  </CardInformation>
                  <a href={ds.storySourceURL} target="_blank" rel="noreferrer">
